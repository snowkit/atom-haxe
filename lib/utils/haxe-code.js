...
var REGEX_ENDS_WITH_FUNCTION_KEYWORD = /[^a-zA-Z0-9_]function\s*$/;
        ...
        while (i > 0) {
            ...
            if (c === '"' || c === '\'') {
                ...
                if (number_of_parens > 0) {
                    ...
                    if (number_of_parens == 0 && number_of_unclosed_braces > 0 && REGEX_ENDS_WITH_FUNCTION_KEYWORD.test(text.slice(0, i))) {
    ...
    code_with_empty_comments_and_strings: function(input) {
        ...
        while (i < len) {
            ...
            if (is_in_single_line_comment) {
                ...
                REGEX_BEGINS_WITH_REGEX.lastIndex = -1;
                if (matches = input.substring(i).match(REGEX_BEGINS_WITH_REGEX)) {
                    var match_len = matches[0].length;
                    output += '~/';
                    for (k = 1; k < match_len - 2; k++) {
                        output += ' ';
                    }
                    output += '/';
                    i += match_len;
