...
platform_atom_MessagePanel.get_visible = function() {
	return platform_atom_MessagePanel.visible;
};
platform_atom_MessagePanel.set_sticky = function(value) {
	platform_atom_MessagePanel.sticky = value;
	if(platform_atom_MessagePanel.sticky && platform_atom_MessagePanel.timeout_id != null) {
		global.clearTimeout(platform_atom_MessagePanel.timeout_id);
		platform_atom_MessagePanel.timeout_id = null;
	} else if(!platform_atom_MessagePanel.sticky && platform_atom_MessagePanel.timeout_id == null) platform_atom_MessagePanel.delay_hide();
	return platform_atom_MessagePanel.sticky;
};
platform_atom_MessagePanel.init = function() {
	platform_atom_MessagePanel.view = new lib_atom_MessagePanelView({ title : "Haxe (dev)", closeMethod : "custom_close"});
};
platform_atom_MessagePanel.show = function() {
	if(!platform_atom_MessagePanel.visible) {
		platform_atom_MessagePanel.visible = true;
		platform_atom_MessagePanel.view.attach();
		platform_atom_MessagePanel.view.panel.custom_close = function() {
			platform_atom_MessagePanel.view.panel.hide();
			platform_atom_MessagePanel.hide();
		};
		if(platform_atom_MessagePanel.timeout_id != null) {
			global.clearTimeout(platform_atom_MessagePanel.timeout_id);
			platform_atom_MessagePanel.timeout_id = null;
		}
		if(!platform_atom_MessagePanel.sticky) platform_atom_MessagePanel.delay_hide();
	}
};
platform_atom_MessagePanel.hide = function() {
	if(platform_atom_MessagePanel.visible) {
		platform_atom_MessagePanel.visible = false;
		platform_atom_MessagePanel.set_sticky(false);
...
platform_atom_MessagePanel.delay_hide = function() {
	var delay = 5000;
	platform_atom_MessagePanel.timeout_id = global.setTimeout(function() {
		platform_atom_MessagePanel.timeout_id = null;
		platform_atom_MessagePanel.hide();
	},delay);
};
...
plugin_Plugin.init = function(serialized_state) {
	...
	plugin_Plugin.subscriptions = new atom_CompositeDisposable();
	plugin_Plugin.init_commands();
	plugin_Plugin.init_menus();
};
plugin_Plugin.dispose = function() {
	plugin_Plugin.subscriptions.dispose();
	plugin_Plugin.state.destroy();
	plugin_Plugin.state = null;
	plugin_Plugin.workers.destroy();
	plugin_Plugin.workers = null;
};
plugin_Plugin.init_commands = function() {
	console.log(platform_atom_Log.format("Init commands"));
	plugin_Plugin.register_command("set-hxml-file",new commands_atom_SetHXMLFileFromTreeView());
};
plugin_Plugin.init_menus = function() {
	console.log(platform_atom_Log.format("Init menus"));
	atom.contextMenu.add({ '.tree-view .file' : [{ type : "separator"},{ label : "Set as active HXML file (dev)", command : "haxe-dev:set-hxml-file", shouldDisplay : plugin_Plugin.should_display_context_tree},{ type : "separator"}]});
};
plugin_Plugin.should_display_context_tree = function(event) {
	var key = ".hxml";
	var val = event.target.innerText;
	if(val == null) return false;
	return StringTools.endsWith(val,key);
};
plugin_Plugin.register_command = function(name,command,module) {
	if(module == null) module = "haxe-dev";
	plugin_Plugin.subscriptions.add(atom.commands.add("atom-workspace",module + ":" + name,function(opts) {
...
platform_atom_MessagePanel.visible = false;
platform_atom_MessagePanel.sticky = false;
platform_atom_ParentProcess.is_kept_alive = false;
