...
HaxeDevMain.serialize = function() {
	if(!HaxeDevMain.failed) return { plugin : plugin_Plugin.state.serialize(), support : support_Support.state.serialize(), 'hello' : "hello"};
	return null;
};
...
commands_atom_SetHXMLFileFromTreeView.prototype = $extend(utils_Command.prototype,{
	execute: function(resolve,reject) {
		...
		platform_atom_Log.info("Active HXML file set to " + plugin_Plugin.state.consumer.hxml_file,null);
		resolve(true);
	}
