...
var REGEX_IS_IDENTIFIER = /^([A-Za-z_][\.A-Za-z0-9_]+)$/;
...
module.exports = {
    ...
    jump: function() {
        ...
        var find_declaration_from_type = function(word) {
            ...
            this._run_haxe_display(display_arg).then(function(result) {
                ...
                xml2js.parseString(result.err, function(err, json) {
                    ...
                    if (json != null) {
                        ...
                        find_declaration_from_toplevel(word);
                    }
        ...
        var find_declaration_from_toplevel = function(word) {
            ...
            this._run_haxe_display(display_arg).then(function(result) {
                ...
                xml2js.parseString(result.err, function(err, json) {
                    ...
                    if (json != null) {
                        ...
                        if (types[word] != null) {
                                // Is this identifier a local declaration?
                            if (types[word] != null && types[word].kind === 'local') {
                                this._open({
                                    file: file_path,
                                    name: word,
                                    local_index: index
                    ...
                    find_declaration_from_raw_word(word);
        ...
        var find_declaration_from_raw_word = function(word) {
        ...
        var find_declaration_from_xml = function(word) {
            ...
            this._extract_info_from_xml_output().then(function(parsed_xml_info) {

                info = parsed_xml_info;

                REGEX_IS_IDENTIFIER.lastIndex = -1;
                if (REGEX_IS_IDENTIFIER.test(word) && word !== 'this') {
                        // Don't try to extract type from the current word
                    find_declaration_from_toplevel(word);
                }
                else {
                        // The current word is not an identifier. Try to resolve to its type
                    find_declaration_from_type(word);
                }
    ...
    _open: function(info) {
        ...
        if (typeof(info) == 'string') {
            ...
            if (info.line == null && info.local_index != null) {
                var cleaned_contents = code.code_with_empty_comments_and_strings(contents).slice(0, info.local_index);

                var regex = new RegExp('(var|\\?|,|\\(|function)\\s*' + identifier + '[^A-Za-z0-9_]', 'g');
                var matches = [];
                cleaned_contents.replace(regex, function() {
                ...
                for (var i = matches.length - 1; i >= 0; i--) {
                    ...
                    var index = matches[i][2] + 1;

                    var lines = contents.slice(0, index).split("\n");
                    var line = lines.length - 1;
                        // Jump to variable/function declaration
                    if (prefix === 'var' || prefix === 'function') {
                        var column = lines[lines.length - 1].length + matches[i][0].length - identifier.length - 2;
                        this._update_editor(file_path, line, column);
                    }
                    else {
                        var signature_info = code.parse_partial_signature(contents, index, {parse_declaration: true});
                ...
                if (match = target_line.match(regex)) {
                    ...
                    this._update_editor(file_path, info.line - 1);
                }
            }
            else {
    ...
    _update_editor: function(file_path, line, column) {

        var params = {};
        if (line != null) params.initialLine = line;
        if (column != null) params.initialColumn = column;

        atom.workspace.open(file_path, params).then(function() {
    ...
    _parse_toplevel_list: function(json) {
        ...
        for (var i = 0; i < json.il.i.length; i++) {
            ...
            if (types[name] == null) {
                types[name] = {
                    type:   (String(raw_entry.$.t || raw_entry.$.p).trim()),
                    kind:   raw_entry.$.k
                };
            }
        }
