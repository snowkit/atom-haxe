...
module.exports = {
    ...
    jump: function() {
        ...
        var find_declaration = function(word) {

            this._extract_info_from_xml_output().then(function(info) {

                    // Suery haxe to get type
                ...
                run.haxe(args).then(function(result) {
                    ...
                    xml2js.parseString(result.err, function(err, json) {
                        ...
                        if (json != null) {
                            ...
                            args.push('--display');
                            args.push(file_path + '@' + Buffer.byteLength(text.slice(0, index_for_toplevel), 'utf8') + '@toplevel');
                                ...
                                xml2js.parseString(result.err, function(err, json) {
                                    ...
                                    if (info[word] != null) {
                                        ...
                                        if (word.lastIndexOf('.') != -1) {
                                            find_declaration(word.slice(0, word.lastIndexOf('.')));
                                        }
                                    }
    ...
    _open: function(info) {
        if (typeof(info) == 'string') {
            ...
            var cleaned_contents = code.code_with_empty_comments_and_strings(String(fs.readFileSync(info.file)));
            var regex = new RegExp(info.kind + '\\s' + identifier);
            var match;
            if (match = cleaned_contents.match(regex)) {
                var index = cleaned_contents.indexOf(match[0]);
                var lines = cleaned_contents.slice(0, index).split("\n");
                var line = lines.length - 1;
                var column = lines[lines.length - 1].length + match[0].length - identifier.length;

                    // We found it, let's open at the exact location
                atom.workspace.open(info.file, {
                    initialLine: line,
