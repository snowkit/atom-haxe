...
var REGEX_ADDITIONAL_KEY_PATH = /(\.[A-Za-z0-9_]+)+(,|;|\(|\)|\s)?$/;
...
module.exports = {
    ...
    jump: function() {
        ...
        var expr_end = code.parse_end_of_expression(text, index);
            ...
            // If the expression end is just a key path, remove it as we want to query the requested word only
        REGEX_ADDITIONAL_KEY_PATH.lastIndex = -1;
        expr_end = expr_end.replace(REGEX_ADDITIONAL_KEY_PATH, '.');
    ...
    _open: function(info) {
        ...
        if (typeof(info) == 'string') {
            ...
            if (info.line == null && info.local_index != null) {
                var index = code.find_local_declaration(contents, identifier, info.local_index);
            ...
            run.haxe(args).then((function(result) {
                ...
                xml2js.parseString(content, function (err, json) {
                    ...
                    for (var i = 0; i < kinds.length; i++) {
                        ...
                        if (kind_entries != null && kind_entries.length > 0) {
