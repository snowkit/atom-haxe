...
var HaxeDevPlugin = function() { };
$hxClasses["HaxeDevPlugin"] = HaxeDevPlugin;
HaxeDevPlugin.__name__ = ["HaxeDevPlugin"];
HaxeDevPlugin.main = function() {
	platform_atom_Log.console.debug(platform_atom_Log.format("Starting HaxeDev plugin..."));
	module.exports = HaxeDevPlugin;
	state_State.init();
	state_State.get_child_worker().run_task(new tasks_HelloTask({ name : "Jérémy"}));
};
HaxeDevPlugin.activate = function(state) {
	HaxeDevPlugin.subscriptions = new atom_CompositeDisposable();
	platform_atom_Log.console.debug(platform_atom_Log.format("Starting HaxeDev worker..."));
	HaxeDevPlugin.subscriptions.add(atom.commands.add("atom-workspace",{ 'haxe-dev:toggle' : HaxeDevPlugin.toggle}));
};
HaxeDevPlugin.deactivate = function(state) {
	HaxeDevPlugin.subscriptions.dispose();
};
HaxeDevPlugin.serialize = function() {
	return { };
};
HaxeDevPlugin.toggle = function() {
	platform_atom_Log.console.log(platform_atom_Log.format("HaxeDev was toggled!"));
};
...
haxe_Unserializer.prototype = {
	...
	,unserialize: function() {
		...
		case 115:
			...
			var bytes = new haxe_io_Bytes(new js_node_buffer_Buffer(size));
			var bpos = 0;
			while(i1 < max) {
...
platform_atom_ChildProcess.prototype = {
	...
	,destroy: function() {
		...
		try {
			...
			platform_atom_Log.console.error(platform_atom_Log.format("Failed to kill child process."));
		}
		this.proc = null;
	}
	,start_proc: function() {
		var _g1 = this;
		var spawn_env;
		if(this.process.platform == "linux") spawn_env = Object.create(this.process.env); else spawn_env = { };
		spawn_env.ATOM_SHELL_INTERNAL_RUN_AS_NODE = "1";
		var node = this.process.execPath;
		var jsfile = __filename;
		if(StringTools.endsWith(jsfile,"-plugin.js")) jsfile = jsfile.substring(0,jsfile.length - 10) + "-worker.js";
		this.proc = js_node_ChildProcess.spawn(node,[jsfile,"has_parent_process"],{ cwd : this.process.cwd(), env : spawn_env, stdio : ["ipc"]});
		this.proc.on("message",function(message) {
			...
			case 0:
				...
				break;
			case 2:
				platform_atom_Log.console.debug(platform_atom_Log.format(message.data));
				break;
			case 3:
				platform_atom_Log.console.log(platform_atom_Log.format(message.data));
				break;
			case 4:
				platform_atom_Log.console.warn(platform_atom_Log.format(message.data));
				break;
			case 5:
				platform_atom_Log.console.error(platform_atom_Log.format(message.data));
				break;
			case 1:
				_g1.ready = true;
		...
		this.proc.on("error",function(error) {
			...
			platform_atom_Log.console.error(platform_atom_Log.format(error));
			_g1.proc = null;
		});
		this.proc.stdout.on("data",function(data) {
			platform_atom_Log.console.debug(platform_atom_Log.format(data));
		});
		this.proc.stderr.on("data",function(data1) {
			platform_atom_Log.console.error(platform_atom_Log.format(data1));
		});
		this.proc.on("close",function(code) {
			if(_g1.destroyed) return;
			if(code == 100) _g1.start_proc(); else if(_g1.got_node_enoent_error) platform_atom_Log.console.error(platform_atom_Log.format("Cannot start child process because of ENOENT error.")); else {
				platform_atom_Log.console.warn(platform_atom_Log.format("Restarting child process. Don't know why it stopped with code: " + code));
				_g1.start_proc();
			}
		});
	}
...
platform_atom_Log.info = function(data) {
	platform_atom_Log.console.log(platform_atom_Log.format(data));
};
platform_atom_Log.warn = function(data) {
	platform_atom_Log.console.warn(platform_atom_Log.format(data));
};
platform_atom_Log.error = function(data) {
	platform_atom_Log.console.error(platform_atom_Log.format(data));
};
...
platform_atom_ParentProcess.has_parent_process = function() {
	return HxOverrides.indexOf(platform_atom_ParentProcess.process.argv,"has_parent_process",0) != -1;
};
...
platform_atom_ParentProcess.post_message = function(message) {
	platform_atom_ParentProcess.initialize_if_needed();
	platform_atom_ParentProcess.process.send({ kind : 0, data : message});
};
platform_atom_ParentProcess.initialize_if_needed = function() {
	if(!platform_atom_ParentProcess.has_parent_process()) {
		platform_atom_Log.console.error(platform_atom_Log.format("Invalid call of ParentProcess: there is no parent process to send/receive message."));
		return;
	}
	platform_atom_ParentProcess.keep_alive();
	platform_atom_ParentProcess.process.on("message",function(message) {
		if(message.kind == 0) {
			var _g = 0;
	...
	platform_atom_ParentProcess.process.send({ kind : 1});
};
platform_atom_ParentProcess.keep_alive = function() {
	if(platform_atom_ParentProcess.is_kept_alive) return;
	platform_atom_Log.console.debug(platform_atom_Log.format("Keep child process alive."));
	...
	platform_atom_ParentProcess.keep_alive_timer.run = function() {
		if(!platform_atom_ParentProcess.process.connected) {
			platform_atom_ParentProcess.is_kept_alive = false;
			platform_atom_ParentProcess.process.exit(ChildProcessExitCode.EXIT_ORPHAN);
		}
	};
...
var state_State = function() { };
$hxClasses["state.State"] = state_State;
state_State.__name__ = ["state","State"];
state_State.get_main_worker = function() {
	return state_State.main_worker;
};
state_State.get_child_worker = function() {
	return state_State.child_worker;
};
state_State.get_parent_worker = function() {
	return state_State.parent_worker;
};
state_State.init = function() {
	var has_parent_process = false;
	has_parent_process = platform_atom_ParentProcess.has_parent_process();
	if(has_parent_process) {
		state_State.main_worker = new utils_Worker({ process_kind : 0});
		state_State.parent_worker = new utils_Worker({ process_kind : 2, main_worker : state_State.get_main_worker()});
	} else {
		state_State.main_worker = new utils_Worker({ process_kind : 0});
		state_State.child_worker = new utils_Worker({ process_kind : 1, main_worker : state_State.get_main_worker()});
	}
};
state_State.synchronize = function() {
};
var utils_WorkerTask = function(params) {
	this.id = utils_WorkerTask.next_id++;
	this.params = params;
};
$hxClasses["utils.WorkerTask"] = utils_WorkerTask;
utils_WorkerTask.__name__ = ["utils","WorkerTask"];
utils_WorkerTask.prototype = {
	get_id: function() {
		return this.id;
	}
	,internal_run: function(resolve,reject) {
		var _g = this;
		this.run(function(r) {
			_g.result = r;
			resolve(_g);
	...
	,__class__: utils_WorkerTask
};
var tasks_HelloTask = function(params) {
	utils_WorkerTask.call(this,params);
};
$hxClasses["tasks.HelloTask"] = tasks_HelloTask;
tasks_HelloTask.__name__ = ["tasks","HelloTask"];
tasks_HelloTask.__super__ = utils_WorkerTask;
tasks_HelloTask.prototype = $extend(utils_WorkerTask.prototype,{
	run: function(resolve,reject) {
		platform_atom_Log.console.log(platform_atom_Log.format("Hello " + this.params.name));
		resolve({ value : "Hello " + this.params.name});
	}
	,__class__: tasks_HelloTask
});
...
var utils_Worker = function(options) {
	this.awaiting_task_callbacks = new haxe_ds_IntMap();
	...
	if(this.process_kind != 0) {
		if(options.main_worker != null) this.main_worker = options.main_worker; else throw new js__$Boot_HaxeError("Option `main_worker` is required on workers of kind CHILD or PARENT");
	} else if(options.main_worker != null) throw new js__$Boot_HaxeError("Option `main_worker` is forbidden on worker of kind MAIN");
...
utils_Worker.prototype = {
	...
	,run_task: function(task) {
		var deferred = new promhx_Deferred();
		var promise = new promhx_Promise(deferred);
		if(this.process_kind == 1) {
			this.await_task_response(task.get_id(),$bind(deferred,deferred.resolve),$bind(promise,promise.reject));
			var serializer = new haxe_Serializer();
			serializer.serialize(2);
			serializer.serialize(task);
			this.child_process.post_message(serializer.toString());
		} else if(this.process_kind == 2) {
			this.await_task_response(task.get_id(),$bind(deferred,deferred.resolve),$bind(promise,promise.reject));
			var serializer1 = new haxe_Serializer();
			serializer1.serialize(2);
			serializer1.serialize(task);
			platform_atom_ParentProcess.post_message(serializer1.toString());
		} else task.internal_run($bind(deferred,deferred.resolve),$bind(promise,promise.reject));
		return promise;
	}
	,destroy: function() {
		if(this.child_process != null) {
			this.child_process.destroy();
			this.child_process = null;
		}
	}
	,await_task_response: function(task_id,resolve,reject) {
		this.awaiting_task_callbacks.h[task_id] = { resolve : resolve, reject : reject};
	}
	,on_process_message: function(message) {
		...
		if(message_kind == 2) {
			...
			this.main_worker.run_task(task).then(function(result) {
				var serializer = new haxe_Serializer();
			...
			var task1 = unserializer.unserialize();
			if(!this.awaiting_task_callbacks.h.hasOwnProperty(task1.id)) throw new js__$Boot_HaxeError("Cannot resolve task " + Std.string(task1) + " because it is not running.");
			var callbacks = this.awaiting_task_callbacks.h[task1.id];
			this.awaiting_task_callbacks.remove(task1.id);
			callbacks.resolve(task1);
		} else if(message_kind == 1) {
			var task2 = unserializer.unserialize();
			if(!this.awaiting_task_callbacks.h.hasOwnProperty(task2.id)) throw new js__$Boot_HaxeError("Cannot reject task " + Std.string(task2) + " because it is not running.");
			var callbacks1 = this.awaiting_task_callbacks.h[task2.id];
			this.awaiting_task_callbacks.remove(task2.id);
...
var utils_WorkerTaskValidator = function() { };
$hxClasses["utils.WorkerTaskValidator"] = utils_WorkerTaskValidator;
utils_WorkerTaskValidator.__name__ = ["utils","WorkerTaskValidator"];
...
platform_atom_Log.console = console;
platform_atom_Log.process = process;
platform_atom_ParentProcess.process = process;
platform_atom_ParentProcess.is_kept_alive = false;
platform_atom_ParentProcess.message_handlers = [];
promhx_base_AsyncBase.id_ctr = 0;
promhx_base_EventLoop.queue = new List();
utils_WorkerTask.next_id = 0;
...
//# sourceMappingURL=haxe-dev-plugin.js.map