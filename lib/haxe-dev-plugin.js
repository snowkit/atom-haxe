...
HaxeDevPlugin.main = function() {
	...
	state_State.init();
	state_State.get_child_worker().run_task(new tasks_HelloTask({ name : "Jérémy"})).then(function(task) {
		platform_atom_Log.console.debug(platform_atom_Log.format("RESULT: " + Std.string(task.result)));
	}).error(function(error) {
		platform_atom_Log.console.error(platform_atom_Log.format("ERROR TASK: " + error));
	});
};
...
tasks_HelloTask.prototype = $extend(utils_WorkerTask.prototype,{
	run: function(resolve,reject) {
		...
		throw new js__$Boot_HaxeError("SOME ERROR");
	...
	,run_task: function(task) {
		...
		return new utils_Promise(function(resolve,reject) {
			if(_g.process_kind == 1) {
				_g.await_task_response(task.get_id(),resolve,reject);
			...
			} else task.internal_run(resolve,reject);
		});
	}
	,destroy: function() {
