...
var HaxeDevWorker = function() { };
$hxClasses["HaxeDevWorker"] = HaxeDevWorker;
HaxeDevWorker.__name__ = ["HaxeDevWorker"];
HaxeDevWorker.main = function() {
	platform_atom_Log.process.send({ kind : 2, data : "Starting HaxeDev worker..."});
	state_State.init();
};
...
platform_atom_ChildProcess.prototype = {
	...
	,kill: function() {
		...
		try {
			...
			platform_atom_Log.process.send({ kind : 5, data : "Failed to kill child process."});
		}
		this.proc = null;
	}
	,start_proc: function() {
		...
		this.proc.on("message",function(message) {
			...
			case 0:
				...
				break;
			case 2:
				platform_atom_Log.process.send({ kind : 2, data : message.data});
				break;
			case 3:
				platform_atom_Log.process.send({ kind : 3, data : message.data});
				break;
			case 4:
				platform_atom_Log.process.send({ kind : 4, data : message.data});
				break;
			case 5:
				platform_atom_Log.process.send({ kind : 5, data : message.data});
				break;
			case 1:
				_g1.ready = true;
		...
		this.proc.on("error",function(error) {
			...
			platform_atom_Log.process.send({ kind : 5, data : error});
			_g1.proc = null;
		});
		this.proc.stdout.on("data",function(data) {
			platform_atom_Log.process.send({ kind : 2, data : data});
		});
		this.proc.stderr.on("data",function(data1) {
			platform_atom_Log.process.send({ kind : 5, data : data1});
		});
		this.proc.on("close",function(code) {
			if(_g1.killed) return;
			if(code == 100) _g1.start_proc(); else if(_g1.got_node_enoent_error) platform_atom_Log.process.send({ kind : 5, data : "Cannot start child process because of ENOENT error."}); else {
				platform_atom_Log.process.send({ kind : 4, data : "Restarting child process. Don't know why it stopped with code: " + code});
				_g1.start_proc();
			}
		});
	}
...
platform_atom_Log.info = function(data) {
	platform_atom_Log.process.send({ kind : 3, data : data});
};
platform_atom_Log.warn = function(data) {
	platform_atom_Log.process.send({ kind : 4, data : data});
};
platform_atom_Log.error = function(data) {
	platform_atom_Log.process.send({ kind : 5, data : data});
};
...
platform_atom_ParentProcess.initialize_if_needed = function() {
	if(!platform_atom_ParentProcess.has_parent_process()) {
		platform_atom_Log.process.send({ kind : 5, data : "Invalid call of ParentProcess: there is no parent process to send/receive message."});
		return;
	}
...
platform_atom_ParentProcess.keep_alive = function() {
	...
	platform_atom_Log.process.send({ kind : 2, data : "Keep child process alive."});
...
tasks_HelloTask.prototype = $extend(utils_WorkerTask.prototype,{
	run: function(resolve,reject) {
		platform_atom_Log.process.send({ kind : 3, data : "Hello " + this.params.name});
...
//# sourceMappingURL=haxe-dev-worker.js.map