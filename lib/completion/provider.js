...
module.exports = {
            ...
            if(!has_list && !has_type) {
                ...
                xml2js.parseString(content, function (err, json) {
                    ...
                    for(var i = 0; i < items.length; ++i) {
                        ...
                        if(item.info.ret) {
                            right = item.info.sig + ':' + item.info.ret;
                        }
    ...
    parseJSON: function(json) {
        ...
        if(json.list) {
            ...
            var info = signatures.parseType(argtypes);
            for(var i = 0; i < info.args.length; ++i) {
                var arg = info.args[i];
                var parts = arg.split(':').map(function(a){ return a.trim(); });
                list.push({ name:parts[0], type:parts[1] });
            }
        }
        ...
        for(var idx = 0; idx < cnt; ++idx) {
            ...
            results.push({
                name:item.name,
                info:signatures.parseType(item.type)
            });
