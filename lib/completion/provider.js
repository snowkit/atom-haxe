...
module.exports = {
    ...
    use_extended_completion: false,

        // Set to true to save into an external file
        // and avoid touching the original file
    ...
    getSuggestions : function(opt) {
        ...
        return new Promise( function(resolve, reject) {
            ...
            if (this.use_external_file) {
                ...
                save_info = file.save_tmp_file_for_completion_of_original_file(opt.editor.buffer.file.path, pretext);
    ...
    onDidInsertSuggestion: function(options) {
        ...
        if (this.use_extended_completion && this.last_key_path != null) {
            editor.insertText(': ');
                ...
                xml2js.parseString(content, function (err, json) {
                    ...
                    if (this.use_extended_completion && position_info != null && position_info.call != null && position_info.call.key_path != null) {
                        ...
                        if (info.type != null && info.type.args != null) {
                            ...
                            structure_type = signatures.string_from_parsed_type(structure_type);
                            ...
                            var fetch = query.get({
                                file: save_info.file_path,
                                byte: save_info.contents.length,
                                add_args: ['-cp', save_info.cp_path]
                            });
            ...
            for (i = 0; i < prev_suggestions.length; i++) {
                ...
                if (prev_suggestion.text == key) {
                    ...
                    if (prev_suggestion.rightLabel != null) {
                        ...
                        return [{
                            rightLabelHTML:     '<span class="current-argument">' + escape(prev_suggestion.rightLabel) + '</span>',
                            text:               '', // No text as we will perform no replacemennt and only label will be visible
                            replacementPrefix:  '',
                            className:          'haxe-autocomplete-suggestion-type-hint'
                        }];
