    ...
    _parse_haxe_completion_json: function(json) {
        ...
        if (json.list) {
            ...
            for (var i = 0; i < json.il.i.length; i++) {
                ...
                var description = raw_entry.d || null;
    ...
    _add_suggestions_with_list: function(list, suggestions, options) {
        ...
        for (var i = 0; i < list.length; ++i) {
            ...
            var type = code.parse_composed_type(item.type);
                ...
                // Detect method kind from toplevel completion
            if (item.kind == 'member' && type.args != null) {
                item.kind = 'method';
            }

            var left = null, right = null, text = null, snippet = null, kind = null;
            ...
            if (type.args != null) {
                ...
                for (var j = 0; j < type.args.length; j++) {
                    ...
                    if (arg != null) {
                        ...
                        if (number_of_chars > 80) {
                            arg_str = arg_str.slice(0, arg_str.length - number_of_chars + 82);
                            if (arg_str.length > 2) {
                ...
                left = code.string_from_parsed_type(type.type);
                ...
                left = code.string_from_parsed_type(type);
                text = name;
            }

                // Don't display Unknown types
            if (left === 'Unknown' || left.slice(0, 8) === 'Unknown<') {
                left = '';
            }
            ...
            switch (item.kind)
            {
                ...
                case 'static':
                    kind = 'property';
                    icon_html = '<span class="icon-letter">s</span>';
                    break;
                case 'type':
                case 'enum':
                    kind = 'type';
            ...
            var completion_display_style = atom.config.get('haxe.completion_display_return_type_position');

            if (completion_display_style == 'right') {
                right = left;
                left = null;
            }
            ...
            if (left != null) {
                suggestion.leftLabel = left;
            }

            if (right != null) {
                suggestion.rightLabel = right;
            }
    ...
    _filter_suggestions: function(prev_suggestions, prev_suggestions_kind, options, position_info) {
        ...
        for (i = 0; i < prev_suggestions.length; i++) {
            ...
            if (prev_suggestion.type != null) {
                ...
                // type
            if (prev_suggestion.iconHTML != null) {
                suggestion.iconHTML = prev_suggestion.iconHTML;
            }
                // label (html)
