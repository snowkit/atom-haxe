...
platform_atom_ChildProcess.prototype = {
	...
	,kill: function() {
		...
		try {
			...
			process.send({ kind : 6, data : "Failed to kill child process.", display : true});
		}
		this.proc = null;
	}
	,start_proc: function() {
		...
		this.proc.on("message",function(message) {
			...
			case 5:
				...
				break;
			case 6:
				process.send({ kind : 6, data : message.data, display : message.display});
				break;
			case 1:
				_g1.ready = true;
		...
		this.proc.on("error",function(error) {
			...
			process.send({ kind : 6, data : error, display : true});
			_g1.proc = null;
		});
		...
		this.proc.stderr.on("data",function(data1) {
			process.send({ kind : 6, data : data1, display : true});
		});
		this.proc.on("close",function(code) {
			if(_g1.killed) return;
			if(code == 100) _g1.start_proc(); else if(_g1.got_node_enoent_error) process.send({ kind : 6, data : "Cannot start child process because of ENOENT error.", display : true}); else {
				process.send({ kind : 5, data : "Restarting child process. Don't know why it stopped with code: " + code, display : true});
				_g1.start_proc();
			}
		});
	}
...
platform_atom_Log.error = function(data,display) {
	if(display == null) display = true;
	process.send({ kind : 6, data : data, display : display});
};
...
platform_atom_ParentProcess.initialize_if_needed = function() {
	if(!platform_atom_ParentProcess.has_parent_process()) {
		process.send({ kind : 6, data : "Invalid call of ParentProcess: there is no parent process to send/receive message.", display : true});
		return;
	}
...
var plugin_Background = function() { };
$hxClasses["plugin.Background"] = plugin_Background;
plugin_Background.__name__ = ["plugin","Background"];
plugin_Background.init = function(serialized_state) {
	process.send({ kind : 2, data : "Starting background worker..."});
	plugin_Background.workers = new plugin_Workers();
};
plugin_Background.dispose = function() {
	plugin_Background.workers.destroy();
	plugin_Background.workers = null;
};
