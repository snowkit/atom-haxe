...
var HaxeDevBackground = function() { };
$hxClasses["HaxeDevBackground"] = HaxeDevBackground;
HaxeDevBackground.__name__ = ["HaxeDevBackground"];
HaxeDevBackground.main = function() {
	process.send({ kind : 2, data : "Starting HaxeDev worker..."});
	state_State.init();
};
...
platform_atom_ChildProcess.prototype = {
	...
	,kill: function() {
		...
		try {
			...
			process.send({ kind : 5, data : "Failed to kill child process.", display : true});
		}
		this.proc = null;
	}
	,start_proc: function() {
		...
		this.proc.on("message",function(message) {
			...
			case 0:
				...
				break;
			case 2:
				process.send({ kind : 2, data : message.data});
				break;
			case 3:
				process.send({ kind : 3, data : message.data, display : message.display});
				break;
			case 4:
				process.send({ kind : 4, data : message.data, display : message.display});
				break;
			case 5:
				process.send({ kind : 5, data : message.data, display : message.display});
				break;
			case 1:
				_g1.ready = true;
		...
		this.proc.on("error",function(error) {
			...
			process.send({ kind : 5, data : error, display : true});
			_g1.proc = null;
		});
		this.proc.stdout.on("data",function(data) {
			process.send({ kind : 2, data : data});
		});
		this.proc.stderr.on("data",function(data1) {
			process.send({ kind : 5, data : data1, display : true});
		});
		this.proc.on("close",function(code) {
			if(_g1.killed) return;
			if(code == 100) _g1.start_proc(); else if(_g1.got_node_enoent_error) process.send({ kind : 5, data : "Cannot start child process because of ENOENT error.", display : true}); else {
				process.send({ kind : 4, data : "Restarting child process. Don't know why it stopped with code: " + code, display : true});
				_g1.start_proc();
			}
		});
	}
...
platform_atom_Log.info = function(data,display) {
	...
	process.send({ kind : 3, data : data, display : display});
};
platform_atom_Log.warn = function(data,display) {
	if(display == null) display = true;
	process.send({ kind : 4, data : data, display : display});
};
platform_atom_Log.error = function(data,display) {
	if(display == null) display = true;
	process.send({ kind : 5, data : data, display : display});
};
...
platform_atom_ParentProcess.initialize_if_needed = function() {
	if(!platform_atom_ParentProcess.has_parent_process()) {
		process.send({ kind : 5, data : "Invalid call of ParentProcess: there is no parent process to send/receive message.", display : true});
		return;
	}
...
platform_atom_ParentProcess.keep_alive = function() {
	...
	process.send({ kind : 2, data : "Keep child process alive."});
...
var state_base_BackgroundState = function() { };
$hxClasses["state.base.BackgroundState"] = state_base_BackgroundState;
state_base_BackgroundState.__name__ = ["state","base","BackgroundState"];
...
state_State.get_background_worker = function() {
	return state_State.background_worker;
};
state_State.get_current_worker = function() {
	if(state_State.get_background_worker().get_process_kind() == 0) return state_State.get_background_worker(); else return state_State.get_main_worker();
};
state_State.get_other_worker = function() {
	if(state_State.get_background_worker().get_process_kind() == 0) return state_State.get_main_worker(); else return state_State.get_background_worker();
};
state_State.init = function() {
	...
	if(has_parent_process) {
		state_State.background_worker = new utils_Worker({ process_kind : 0});
		state_State.main_worker = new utils_Worker({ process_kind : 2, current_worker : state_State.get_background_worker()});
	} else {
		state_State.main_worker = new utils_Worker({ process_kind : 0});
		state_State.background_worker = new utils_Worker({ process_kind : 1, current_worker : state_State.get_main_worker()});
	}
};
state_State.synchronize = function() {
	var values = { };
	values.hxml_data = state_State.hxml_data;
	state_State.get_other_worker().run_task(new tasks_SynchronizeState({ values : values}));
};
state_State.assign_values = function(values) {
	state_State.hxml_data = values.hxml_data;
};
state_State.__super__ = state_base_BackgroundState;
state_State.prototype = $extend(state_base_BackgroundState.prototype,{
	__class__: state_State
});
...
var tasks_SetHXMLFile = function(params) {
	utils_WorkerTask.call(this,params);
};
$hxClasses["tasks.SetHXMLFile"] = tasks_SetHXMLFile;
tasks_SetHXMLFile.__name__ = ["tasks","SetHXMLFile"];
tasks_SetHXMLFile.__super__ = utils_WorkerTask;
tasks_SetHXMLFile.prototype = $extend(utils_WorkerTask.prototype,{
	run: function(resolve,reject) {
		process.send({ kind : 2, data : "Set HXML File to " + this.params.path});
		state_State.hxml_data = "";
		state_State.synchronize();
		resolve(true);
	}
	,__class__: tasks_SetHXMLFile
});
var tasks_SynchronizeState = function(params) {
	utils_WorkerTask.call(this,params);
};
$hxClasses["tasks.SynchronizeState"] = tasks_SynchronizeState;
tasks_SynchronizeState.__name__ = ["tasks","SynchronizeState"];
tasks_SynchronizeState.__super__ = utils_WorkerTask;
tasks_SynchronizeState.prototype = $extend(utils_WorkerTask.prototype,{
	run: function(resolve,reject) {
		state_State.assign_values(this.params.values);
		resolve(true);
	}
	,__class__: tasks_SynchronizeState
});
...
var __map_reserved = {}
HaxeDevBackground.process = process;
...
HaxeDevBackground.main();
})(typeof console != "undefined" ? console : {log:function(){}}, typeof window != "undefined" ? window : exports, typeof window != "undefined" ? window : typeof global != "undefined" ? global : typeof self != "undefined" ? self : this);

//# sourceMappingURL=haxe-dev-background.js.map