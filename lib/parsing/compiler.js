...
var REGEX_HAXE_COMPILER_OUTPUT_LINE = /^\s*(.+)?(?=\:[0-9]*\:)\:([0-9]+)\:\s+(characters|lines)\s+([0-9]+)\-([0-9]+)(?:\s+\:\s*(.*?))?\s*$/;
    // Match a warning message
var REGEX_HAXE_WARNING_MESSAGE = /^Warning\s*\:\s*(.*)$/;
...
module.exports = {
    ...
    parse_output: function(output, options) {
        ...
        var i, l, m, mtype, line, file_path, location, start, end, message, type;
        ...
        for (i = 0; i < lines.length; i++) {
            ...
            if (m = line.match(REGEX_HAXE_COMPILER_OUTPUT_LINE)) {
                ...
                if (mtype = message.match(REGEX_HAXE_WARNING_MESSAGE)) {
                    type = 'Warning';
                    message = mtype[1];
                }
